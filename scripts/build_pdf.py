...
<div class="box ok" style="margin:5mm 0"><span class="lbl">قيدان يحكمان كل الخطط</span>
<p class="sm"><strong>فصل التخرج:</strong> <span class="code">منطر4600</span> التدريب الميداني
و<span class="code">منطر4400</span> مشروع التخرج يُؤخذان <strong>معاً في الفصل الأخير</strong>،
ولا يُؤخذ معهما أي مقرر آخر — والمسموح الأقصى متطلب جامعة واحد، وهو
<strong>غير مُستعمَل في أي من الخطط الأربع</strong>، فكل فصول التخرج تسعُ ساعات بمقررين اثنين.
...
<p class="sm" style="margin-bottom:6mm"><strong>أثر ذلك على الاستثناءات:</strong> الدوران المنفصل
هو ما يحشر المقررات الخريفية الستّ عشرة في خريف واحد في السيناريو أ — فمن هنا جاء الاستثناءان
معاً: <span class="mono">E1</span> لتزامن المشروع الفني مع متطلبيه، و<span class="mono">E2</span>
لزيادة العبء إلى 16 ساعة. أما الخطط الثلاث الأخرى فتوزّع الخريفيات على خريفين وتستغني عن
الاثنين.</p>
...
<span style="color:var(--ink-3)"> — الفجوة الوحيدة: {ev["gaps"][0]}</span></p>""",
    ...
    audit = [("الفصل الأول: منطر3027 · ترفن4111 · ترفن2130 · ترفن3140 · ترفن3210",
             ...
             ("منطر4400 مشروع التخرج مصاحباً للتدريب",
              "سليم", "ok", "الخطة تنصّ عليه صراحةً، وجدول الجامعة يؤكده",
              "يبقى في فصل التخرج مع التدريب"),
...
<div class="box ok"><span class="lbl">فصل التخرج: التدريب والمشروع معاً</span>
<p>نصّ الشرط الأول نفسه يحسم الأمر: مشروع التخرج <strong>«يتم دراسته كمقرر مصاحب مع مقرر
التدريب الميداني»</strong>. فالفصل الأخير = <span class="code">منطر4600</span> +
<span class="code">منطر4400</span> معاً، ولا يُؤخذ معهما مقرر آخر — والمسموح الأقصى متطلب
جامعة واحد، غير مُستعمَل في أي خطة.</p>
<p class="sm" style="margin-top:2mm">والجدول الرسمي يؤكد أن المصاحبة مقصودة لا محتمَلة:
التدريب يشغل <strong>الأيام الخمسة 08:00–13:50</strong>، ومشروع التخرج
<strong>مساءً حصراً</strong> (14:15–19:05) في الموسمين. عدد الأزواج الخالية من التعارض:
<strong>156</strong> خريفاً و<strong>1424</strong> ربيعاً.</p></div>""",
...
def pages_tail(doc, d):
    ...
    risks = [
      ...
      ("hi", "رفض أحد استثناءي السيناريو أ",
       "أ تحتاج موافقتين لا واحدة: E1 لتزامن المشروع الفني مع متطلبيه، وE2 لزيادة العبء إلى "
       "16 ساعة مع معدل 2.18 (النظام يشترط 3.00). رفض أيٍّ منهما يُسقطك إلى ب — فصل إضافي "
       "وخمسة أشهر. وب لا تحتاج شيئاً أصلاً، فالمقايضة معلومة سلفاً."),
    ...
    checks = [
      "تثبيت شعبتَي فصل التخرج: التدريب الميداني صباحاً ومشروع التخرج مساءً في نفس "
      "الفصل — الأزواج الخالية من التعارض كثيرة (156 خريفاً · 1424 ربيعاً)، لكن يلزم "
      "حجز الشعبتين معاً عند فتح التسجيل.",
      "مواعيد عربي1060 واجمع1005 في الفصول الربيعية: جداول أقسامهما (اللغة العربية "
      "والاجتماع) غير مشمولة في ملفات جدول الربيع المتوفّرة، فتُؤخذ مواعيدهما من "
      "الجدول الرسمي عند التسجيل.",
...
<strong>الموضوع:</strong> طلب استثناءين لاختصار فصل دراسي — تزامن المشروع الفني مع متطلبيه،
وزيادة العبء إلى 16 ساعة</p>
<p>تحية طيبة وبعد،</p>
...
<p>وألتمس من سعادتكم الموافقة على أمرين معاً، إذ لا يُجدي أحدهما دون الآخر:</p>
<p><span class="mono">E1</span> · <strong>تسجيل ترفن4260 متزامناً مع ترفن2110 وترفن3111 في
فصل خريف 2026.</strong><br>
<span class="mono">E2</span> · <strong>زيادة العبء في ذلك الفصل إلى 16 ساعة معتمدة</strong>،
علماً بأن معدلي التراكمي 2.18 ودون العتبة 3.00 الواردة في البند ب-3. والزيادة هنا
<strong>مفروضة لا اختيارية</strong>: مجموع ما تبقّى لي من مقررات خريفية 16 ساعة، وكل بدائل
خانتَي الاختياري (ب) و(د) خريفية كذلك، فلا سبيل إلى توزيعها على خريفين ضمن خطة الفصول الثلاثة.</p>
<p>وبالموافقتين أتخرّج في <strong>يناير 2028</strong> — أي أبكر بفصل دراسي كامل.</p>
...
<p>وفي حال تعذّر أيٌّ من الاستثناءين، فإني ماضٍ في الخطة البديلة (أربعة فصول، تخرّج
يونيو 2028) وهي <strong>لا تتطلب أي استثناء</strong>. وفي الحالتين يبقى الفصل الأخير
مخصَّصاً للتدريب الميداني ومشروع التخرج معاً وفق نصّ الخطة.</p>
...
2025/2026 — الخريف يغطي ARED · CUTM · EDUC · PSYC · TECH · ARAB · SOCY، والربيع يغطي
كلية التربية وحدها (ARED · CUTM · EDUC · PSYC) — · استمارة تدقيق الخطة الدراسية ·
...
مشتقّة آلياً من كشف الدرجات الرسمي (25 أغسطس 2026) ومطابقة لسطر مجاميعه. لا شيء هنا نافذ قبل اعتماد المرشد الأكاديمي
...
def load_meetings():
    ...
    if not MEETINGS_PATH.exists():
        return {}
    MEETINGS_CACHE = json.loads(
        MEETINGS_PATH.read_text(encoding="utf-8"))["meetings"]
...
def seasons_missing_dept(meetings, season, code):
    """هل قسم هذا المقرر غائب كلياً عن ملفات جدول هذا الموسم؟

    ملفات جدول الربيع التي وصلتنا تغطّي كليّة التربية وحدها، فأقسام مثل
    اللغة العربية والاجتماع غائبة عنها بالكامل. غياب المقرر عندئذٍ دليل على
    نقص المصدر لا على عدم الطرح — والخلط بينهما هو ما جعل النسخة السابقة
    تعرض مواعيد خريفية على أنها ربيعية.
    """
    pre = code[:4]
    return not any(k.startswith(f"{season}:{pre}") for k in meetings)
...
def page_whats_changed(doc, d, tr):
    changes = [
      ...
      ("منطر4400 مشروع التخرج", "نُقل إلى الفصل السابق للتدريب",
       "<strong>يعود إلى فصل التخرج مع التدريب</strong> — مصاحبةٌ تنصّ عليها الخطة"),
      ("مواعيد جدول الربيع", "بيانات الخريف منسوخة تحت الربيع لـ160 مقرراً",
       "<strong>أُعيد الاستخراج</strong> بعد إسقاط عشر نسخ مكرَّرة"),
    ]
...
<p>إن كنتَ قد عرضتَ نسخة سابقة على مرشدك أو رئيس القسم، فهذه الصفحة تبيّن ما تغيّر
ولماذا. إخفاء التصحيح أسوأ من إعلانه.</p></div>
<div class="box" style="border-top:2pt solid var(--stamp);margin-bottom:5mm">
<span class="lbl">تصحيح آخر: موضع مشروع التخرج</span>
<p class="sm">نسخة أسبق من هذه الوثيقة نقلت <span class="code">منطر4400</span> مشروع التخرج
إلى الفصل <strong>السابق</strong> للتدريب الميداني. كان ذلك خطأً، وثلاثة مصادر مستقلة تنقضه:
نصّ شرط التدريب في الخطة («يتم دراسته كمقرر <strong>مصاحب</strong> مع مقرر التدريب الميداني»)،
وتسجيل المصاحبة في بيانات المقرر نفسه، وجدول الجامعة الذي يطرح المشروع
<strong>مساءً حصراً</strong> ليجتمع مع تدريبٍ يشغل الصباح. الفصل الأخير الآن في الخطط الأربع =
<span class="code">منطر4600</span> + <span class="code">منطر4400</span> معاً، تسعُ ساعات
بمقررين لا ثالث لهما.</p></div>
<h3>التصحيحات الثمانية</h3>
...
def weekly_block(d, key, term, res):
    ...
    if flex:
        gap = [c for c in res["flexible"]
               if seasons_missing_dept(MEETINGS_CACHE, term["season"], c)]
        rest = [c for c in res["flexible"] if c not in gap]
        if gap:
            legend.append("قسمها خارج ملفات جدول "
                          + SEASON_AR[term["season"]]
                          + " المتوفّرة (الموعد من الجدول الرسمي): "
                          + " · ".join(C[c]["ar"] for c in gap))
        if rest:
            legend.append("بلا وقت في بيانات 2025/2026 (يُختار عند التسجيل): "
                          + " · ".join(C[c]["ar"] for c in rest))
    if not legend:
